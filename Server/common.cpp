...
void split_dos(char *dest, int dst_size, char *message, int msg_size, char char_spliter, int *initial)
{
        int i, a = 0;
...
char *guild_mode_desc ( int mode, char *dest )
{
	...
	switch ( mode )
	{
		...
		case GM_5_ATTDEF: strcpy(dest, "+10% Damage / Defense vs Bots"); break;
		case GM_100_GREWARDS: strcpy(dest, "+100% Guild Rewards"); break;
		case GM_10_DROPS: strcpy(dest, "+10% Chance of Drops"); break;
		case GM_10_ATTDEF: strcpy(dest, "+15% Damage / Defense vs Bots"); break;
		case GM_ULTIMATE: strcpy(dest, "+25% Damage / Defense vs Bots, +12% Drops"); break;
	}
...
double guild_mode_def(int mode, double damage)
{
	switch(mode)
	{
	case GM_5_ATTDEF: damage *= 0.9; break; // 0.95
	case GM_10_ATTDEF: damage *= 0.85; break; // 0.9
	case GM_ULTIMATE: damage *= 0.8; break; // 0.88
	}
...
double guild_mode_att(int mode, double damage)
{
	switch(mode)
	{
	case GM_5_ATTDEF: damage *= 1.1; break; // 1.05
	case GM_10_ATTDEF: damage *= 1.15; break; // 1.1
	case GM_ULTIMATE: damage *= 1.25; break; // 1.12
	}
